...
export function useTopicStorage() {
  const [topicInfo, setTopicInfo, removeTopicInfo, topicLoading] =
    useIDBStorage<TopicInfo>(
      TOPIC_KEY,
      emptyTopicInfo,
      STORE_NAME
    );
  ...
  const updateTopic = async (topic: TopicState, shouldSelect: boolean = true) => {
    try {
      ...
      await setTopicInfo({
        topics: newTopics,
        currentTopicId: shouldSelect ? topic.topicId : topicInfo.currentTopicId,
      });
      return true;
    } catch (error) {
  ...
  const removeTopic = async (topicId: string) => {
    try {
      ...
      await setTopicInfo({
        topics: newTopics,
        currentTopicId:
          topicId === topicInfo.currentTopicId ? '' : topicInfo.currentTopicId,
      });
      return true;
    } catch (error) {
